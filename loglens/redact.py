...
_RULES: tuple[tuple[str, str], ...] = (
    ...
    ("AUTH_HEADER", r"\b(?:Authorization|Proxy-Authorization)\s*[:=]\s*\S+"),
    ("BEARER_TOKEN", r"\b(?:Bearer|Basic|Token)\s+[A-Za-z0-9._~+/=-]{12,}"),
    (
        "SECRET_ASSIGNMENT",
    ...
    ("CARD_NUMBER", r"\b\d(?:[ -]?\d){12,18}\b"),
    ("SSN", r"\b\d{3}-\d{2}-\d{4}\b"),
)

_COMBINED = re.compile(
    "|".join(f"(?P<{kind}>{pattern})" for kind, pattern in _RULES),
    re.IGNORECASE,
)
...
def redact(text: str) -> RedactionResult:
    ...
    return RedactionResult(_COMBINED.sub(replace, text), counts)
